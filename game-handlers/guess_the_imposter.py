...
"Juice", "Milk", "Water", "Soda", "Smoothie", "Milkshake"
    ...
    def start_game(self) -> str:
        ...
        actual_impostors = self.num_impostors
        if len(player_ids) <= 10 and actual_impostors > 2:
            actual_impostors = 2
        if len(player_ids) <= 4:
            actual_impostors = 1
            
        actual_impostors = min(actual_impostors, max(1, len(player_ids) - 1))
        self.imposter_ids = random.sample(player_ids, actual_impostors)
    ...
    def get_player_role(self, user_id: int) -> str:
        ...
        if user_id in self.imposter_ids:
            return "IMPOSTER"
        return self.secret_word
    ...
    def submit_clue(self, user_id: int, clue: str) -> bool:
        ...
        if not hasattr(self, 'clue_history'):
            self.clue_history = []
    ...
    def resolve_game(self) -> dict:
        ...
        imposter_caught = (most_voted in self.imposter_ids) and not tie
        
        imposter_names = [self.players.get(i_id, "Unknown") for i_id in self.imposter_ids]
