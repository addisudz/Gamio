...
class StoryBuilderGame:
    ...
    def __init__(self, rounds_per_player: int = 2):
        ...
        # Load starting prompts from JSON
        json_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "assets", "story-builder", "starting_phrases.json")
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                self.starting_prompts = json.load(f)
        except Exception as e:
            logger.error(f"Failed to load story builder prompts: {e}")
            self.starting_prompts = [
    ...
    def set_options(self, rounds_per_player: int) -> None:
    ...
    def start_game(self) -> str:
        ...
        if "[name]" in start_text:
            random_player_id = random.choice(self.players)
            random_player_name = self.player_names[random_player_id].split()[0]  # First name only
            mention = f'<a href="tg://user?id={random_player_id}">{random_player_name}</a>'
            start_text = start_text.replace("[name]", mention)
